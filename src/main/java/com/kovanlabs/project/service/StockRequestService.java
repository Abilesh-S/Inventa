...
public class StockRequestService {
    ...
    public List<StockRequest> getRequestsForManager(String managerEmail) {
        ...
        if (manager.getRole() != Role.MANAGER || manager.getBranch() == null) {
            throw new RuntimeException("Only managers with an assigned branch can view requests");
        }

        return stockRequestRepository.findByBranchIdOrderByIdDesc(manager.getBranch().getId());
    }
    ...
    public StockRequest approve(Long requestId, String ownerEmail, String remark) {
        ...
        // We only dispatch the item; branch manager must receive it to update their inventory.
    ...
    @Transactional
    public StockRequest receive(Long requestId, String managerEmail) {
        User manager = userRepository.findByEmail(managerEmail)
                .orElseThrow(() -> new RuntimeException("Manager not found"));

        if (manager.getRole() != Role.MANAGER || manager.getBranch() == null) {
            throw new RuntimeException("Only managers can receive requests");
        }

        StockRequest request = stockRequestRepository.findById(requestId)
                .orElseThrow(() -> new RuntimeException("Stock request not found"));

        if (!request.getBranch().getId().equals(manager.getBranch().getId())) {
            throw new RuntimeException("Cannot receive request for another branch");
        }

        if (request.getStatus() != StockRequestStatus.DISPATCHED) {
            throw new RuntimeException("Only dispatched requests can be received");
        }
